...
export default function HeaderBackground({ type, bgColor }: Typeprops) {
  ...
  const range = useMemo(() => ({ default: Math.floor(window.screen.height * 0.4), home: 64 }), [])
  const onScroll = useCallback(
    // eslint-disable-next-line no-unused-vars
    (e: Event) => {
      const scrollHeight = document.querySelector('#main_area')?.scrollTop
      if (scrollHeight) {
  ...
  useEffect(() => {
    ...
    // document.querySelector('#main_area')?.addEventListener('scroll', onScroll)
    document.querySelector('#main_area')?.addEventListener('scroll', throttledOnScroll)
    ...
    // return () => document.querySelector('#main_area')?.removeEventListener('scroll', onScroll)
    return () => document.querySelector('#main_area')?.removeEventListener('scroll', throttledOnScroll)
  }, [])
